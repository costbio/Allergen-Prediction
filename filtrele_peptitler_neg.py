...
# --- Ayarlar ---
klasor_yolu = 'PeptitNeg' # DEĞİŞTİRİLDİ: Negatif peptidlerin olduğu klasör
rank_sutunu_ana_htarif = 'El_Rank' 
aranacak_uzantilar = ('.xls', '.xlsx')
# DEĞİŞTİRİLDİ: Negatif sonuçların toplanacağı tek Excel dosyasının adı
birlesik_cikti_dosyasi_adi = 'Tum_Filtrelenmis_Negatif_Peptidler.xlsx' 
# Bu dosya, script'in çalıştığı dizine kaydedilecektir.

# --- İşleme Fonksiyonu (Aynı Kalıyor) ---
...
try:
    # _filtered.xlsx ile biten dosyaları OKUMAMAK için kontrol
    dosya_adlari = [
        f for f in os.listdir(klasor_yolu) 
        if f.endswith(aranacak_uzantilar) and 
           not f.startswith('~') and 
           not f.endswith('_filtered.xlsx') # Bu satır önemli!
    ]
...
if not dosya_adlari:
    print(f"HATA: '{klasor_yolu}' klasöründe uygun dosya bulunamadı (veya hepsi '_filtered.xlsx' idi).")
    sys.exit(1)
...
dosya_sayaci = 0
basarili_dosya_sayaci = 0 
hatali_dosya_sayaci = 0   
bulunan_peptid_dosya_sayaci = 0 

tum_filtrelenmis_peptid_listesi = [] 
...
for dosya_adi in dosya_adlari:
    ...
    try:
        ...
        try:
            ...
            raise ValueError(f"'{gecerli_rank_sutunu}' sütununa sınıflandırma uygulanırken hata: {e_apply}. Sütundaki bazı değerler: {problematic_values}")

        filtrelenmis_df = df.dropna(subset=['Binding_Strength']).copy()

        if not filtrelenmis_df.empty:
            filtrelenmis_df.loc[:, 'KaynakDosya'] = dosya_adi 
